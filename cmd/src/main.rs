...
/// Indexing the hierarchy with an out-of-range scope id panics, so ids coming
/// from the client are checked against the loaded file first.
fn scope_by_id(state: &AppState, id: u32) -> Result<ScopeRef, AppError> {
    if id >= state.scope_count {
        return Err(AppError::Illegal(id));
    }
    ScopeRef::from_index(id as usize).ok_or(AppError::Illegal(id))
}
...
fn cmd_get_children(state: &AppState, id: u32, start_index: u32) -> Result<rmpv::Value, AppError> {
    ...
    let scope_ref = scope_by_id(state, id)?;
...
fn signal_data(
    ...
    if let Some(ts) = time_start {
        if let Some(offset) = lo.checked_sub(1).and_then(|i| signal.get_offset(indices[i])) {
...
fn cmd_get_signal_data(
    ...
    let (hierarchy, signal_source, time_table) = match (
        state.hierarchy.as_ref(),
        state.signal_source.as_mut(),
        state.time_table.as_ref(),
    ) {
        (Some(h), Some(s), Some(t)) => (h, s, t),
        _ => return send_error(writer, request_id, "No file loaded".to_string()),
    };
...
fn cmd_search(state: &AppState, query: &str, scope_id: u32) -> Result<rmpv::Value, AppError> {
    ...
    let search_scope = if scope_id != 0xFFFFFFFF {
        Some(hierarchy.index(scope_by_id(state, scope_id)?))
    } else {
        None
    };
