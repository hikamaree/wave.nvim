...
if clk_vc then
    local top, bot = SingleBit.paint(clk_vc, TimeScale.new(0, 200, 40))
...
if data_vc then
    local top, bot = MultiBit.paint(data_vc, TimeScale.new(0, 200, 40))
...
if clk_vc then
    local nums, ticks = Ruler.paint(TimeScale.new(0, 200, 40), clk_vc)
...
--- 6. TraceList integration
print("--- 6. TraceList:set_data (number times) ---")

do
  local TraceList = require("wave.model.trace_list")
  local SignalRef = require("wave.model.signal_ref")

  local traces = TraceList.new()
  traces:add(SignalRef.new({ netlist_id = 1, signal_id = 1, path = "top.clk", width = 1 }))
  traces:set_data(1, clk_vc, nil, { 0, 200 })

  local trace = traces:get(1)
  check("trace found", trace ~= nil)
  if trace then
    check("value changes stored", trace.value_changes ~= nil)
    if trace.value_changes and trace.value_changes[1] then
      check("vc[1] is number (from TraceList)", type(trace.value_changes[1][1]) == "number")
    end
  end

  check("duplicate rejected", select(2, traces:add(
    SignalRef.new({ signal_id = 1, path = "top.clk" }))) == "Signal already displayed")

  -- The edge index is derived from the loaded data and rebuilt on change.
  local edges = traces:edge_index()
  check("edge index built", edges:count() > 0)
  local first = edges.times[1]
  check("edge next advances", edges:next(first) ~= nil and edges:next(first) > first)
  check("edge prev of first is nil", edges:prev(first) == nil)
  check("nearest snaps to an edge", edges:nearest(first + 1) ~= nil)

  traces:clear()
  check("cleared", traces:count() == 0)
end
...
--- 8. Chunk merging, exercising Request:merge_chunks for real
print("--- 8. Request chunk-merge unit test ---")

do
  local Request = require("wave.ipc.request")
  local request = Request.new(99, function() end)
  ...
  -- The final frame carries the last entry; it must land after the chunks.
  local resp = {
    success = true,
    request_id = 99,
    data = { { signal_id = 1, value_changes = { { 80, "0" } } } },
  }
  request:merge_chunks(resp)
  ...
  check("final frame data goes last", resp.data[5].value_changes[1][1] == 80)
  check("vc is [number, string]", type(resp.data[1].value_changes[1][1]) == "number")

  -- No chunks: the response must be left exactly as it arrived.
  local plain = Request.new(100, function() end)
  local solo = { success = true, data = { { signal_id = 7 } } }
  plain:merge_chunks(solo)
  check("unchunked response untouched", #solo.data == 1 and solo.data[1].signal_id == 7)
end
...
if clk_vc then
  ...
  local lines = ValueTable.paint(stub, 22)
...
do
  ...
  -- Synchronous helper over the client's callback-based request
  local function psend(cmd)
    local resp, done = nil, false
    p:request(cmd, function(r) resp = r; done = true end)
